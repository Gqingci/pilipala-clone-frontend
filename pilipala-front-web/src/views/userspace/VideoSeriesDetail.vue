    ...
    <div class="detail-panel">
      <div class="video-detail">
        <div class="count-info">
          <div>
            {{ videoList.filter((v) => v.seriesId !== "add").length }}个视频
          </div>
      ...
      <template v-if="myself">
        <el-button type="primary" class="edit-btn" @click="editSeries"
          >编辑</el-button
        >
        <el-button type="primary" class="del-btn" @click="delSeries"
          >删除</el-button
        >
      </template>
    </div>
    ...
    >
      <template v-for="(item, index) in videoList" :key="item.seriesId">
        ...
        <div class="list-item" v-else>
          ...
          <div class="list-name">{{ item.videoName }}</div>
          <div class="play-count-info">
            <div class="play-count iconfont icon-bofangliang">
              {{ item.playCount }}
            </div>
...
const getSeriesDetail = async () => {
  ...
  videoList.value = result.data.userVideoSeriesVideoList;
  if (myself.value) {
